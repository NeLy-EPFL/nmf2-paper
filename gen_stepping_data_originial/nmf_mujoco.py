...
from data import mujoco_groundwalking_model_path
from data import default_pose_path, stretch_pose_path
from config import all_leg_dofs
...
_default_terrain_config = {
    'flat': {'size': (50_000, 50_000),
             'fly_placement': ((0, 0, 300), (0, 1, 0, 0.1))},
...
_default_render_config = {
    'saved': {'window_size': (640, 480), 'playspeed': 1.0, 'fps': 60, 'camera_id':-1},
    'headless': {'window_size': (np.nan, np.nan), 'playspeed': np.nan, 'fps': np.nan, 'camera_id':np.nan},
}
...
class NeuroMechFlyMuJoCo(gym.Env):
    ...
    def __init__(self,
        ...
        with open(_init_pose_lookup[init_pose]) as f:
            self.init_pose = {k: np.deg2rad(v)
                         for k, v in yaml.safe_load(f)['joints'].items()}
        self.init_pose = {k: v for k, v in self.init_pose.items() if k in actuated_joints}
        ...
        # This did not work probably due to resetting the joints coordinates
        ...
        self.all_joints = [joint.name for joint in arena.find_all('joint')]
        if self.terrain == 'flat':
            self.all_bodies = [body.name for body in arena.find_all('body')[1:]]
        else:
            self.all_bodies = [body.name for body in arena.find_all('body')]
        self.all_geom = [geom.name for geom in arena.find_all('geom')]
    ...
    def set_init_pose(self, init_pose: Dict[str, float]):
        ### With the code line 115 input the joint angles is not working (legs get crumbled up no coordination)
        ### With the code below the input joint angles are working (legs are coordinated)
        with self.physics.reset_context():
            for i in range(len(self.actuated_joints)):
                if self.actuators[i].joint.name in self.actuated_joints and self.actuators[i].joint.name in init_pose:
                    angle_0 = init_pose[self.actuators[i].joint.name]
                    self.physics.named.data.qpos["Animat/" + self.actuators[i].joint.name] = angle_0
    ...
    def step(self, action: ObsType
        ...
        if 'joints' in action:
            ...
            for act in self.actuators:
                self.physics.bind(act).ctrl = action[act.joint.name]
        self.physics.step()
